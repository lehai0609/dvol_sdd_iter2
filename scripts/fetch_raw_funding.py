...
from dvol_data.etl_funding import daily_funding_from_intraday  # noqa: E402
...
def persist_partitions(
    ...
    *,
    ...
    """Deprecated here: kept for parity but not used for funding CSV output.

    We still compute partition counts for diagnostics but do not write Parquet.
    """
    if df.empty:
        print(f"No rows to write for {table} ({layer})")
        return []
    grouped = df.groupby([asset_col, date_col], dropna=False, sort=False)
    count = sum(1 for _ in grouped)
    print(f"Computed {count} partitions for {table} ({layer}) [no Parquet write]")
    return []


def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch raw funding from CDD and persist partitions.")
    parser.add_argument("--asset", default="BTC", help="Asset symbol for perpetual (e.g., BTC, ETH)")
    ...
    print(f"Asset: {asset} | Symbol: {symbol} | Range: {end_date - timedelta(days=lookback)} -> {end_date} | Limit: {limit}")
    ...
    # Normalize to daily funding contract
    raw_df = pd.DataFrame(result)
    if raw_df.empty:
        df = pd.DataFrame(columns=["asset", "date", "symbol", "interest_8h", "interest_1h", "index_price"])  # type: ignore[list-item]
    else:
        df = daily_funding_from_intraday(raw_df)
        df = df.sort_values("date").reset_index(drop=True)
        df = df[df["asset"].astype(str).str.upper() == asset].reset_index(drop=True)
    ...
    print(f"Funding rows normalized: {len(df)}")
    ...
    # Instead of Parquet partitions, write a single CSV with full schema
    schema_path = CONTRACTS_DIR / "raw_funding.schema.json"
    raw_funding_schema = json.loads(schema_path.read_text(encoding="utf-8"))
    schema_cols = [
        # keep a stable order
        "asset",
        "date",
    ...
    out_dir = RAW_DATA_ROOT / "raw" / "raw_funding"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_name = f"raw_funding_{asset}_{end_date.isoformat()}.csv"
    out_path = out_dir / out_name
    df_out.to_csv(out_path, index=False)

    # Still report diagnostic partition count (without writing them)
    if not df_out.empty:
        part_keys = df_out[["asset", "date"]].dropna().drop_duplicates()
        print(f"Computed partitions (asset/date): {len(part_keys)}")
    else:
        print("Computed partitions (asset/date): 0")

    print(f"Wrote CSV: {out_path} ({len(df_out)} rows, {len(df_out.columns)} cols)")

    # Save fetched JSON payload alongside CSV for inspection
    json_name = f"raw_funding_{asset}_{end_date.isoformat()}.json"
